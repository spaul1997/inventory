...
const navItems = [
  ...
  { to: "/inventory-reports/purchase-report", label: "Purchase Report", icon: ShoppingCart, module: "purchase" },
  { to: "/inventory-reports/stock-valuation", label: "Stock Valuation", icon: Scale },
];

export function InventoryReportsSidebar() {
  const { session } = useAuth();
  const visibleItems = navItems.filter((item) => !item.module || hasCompanyModule(session?.user?.company?.enabledModules, item.module));
