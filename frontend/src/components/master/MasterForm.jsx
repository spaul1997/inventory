...
function normalizeFieldValue(field, value, { trim = false } = {}) {
  if (!field?.uppercase || typeof value !== "string") return value;
  const nextValue = trim ? value.trim() : value;
  return nextValue.toUpperCase();
}

function normalizeEntityValues(entity, values, options) {
  const nextValues = { ...values };
  entity.form.tabs.forEach((tab) => {
    tab.fields.forEach((field) => {
      if (nextValues[field.key] !== undefined) {
        nextValues[field.key] = normalizeFieldValue(field, nextValues[field.key], options);
      }
    });
  });
  return nextValues;
...
export function MasterForm({ entityKey, mode, recordId }) {
  ...
  const [values, setValues] = useState(() => {
    const base = normalizeEntityValues(entity, { ...blankValues(entity), ...(existingRecord || {}) });
  ...
  useEffect(() => {
    if (recordId && existingRecord) {
      setValues(normalizeEntityValues(entity, { ...blankValues(entity), ...existingRecord }));
    }
  ...
  function setField(key, value, field) {
    setValues((prev) => {
      const next = { ...prev, [key]: normalizeFieldValue(field, value) };
  ...
  function collectUniqueFieldErrors() {
    const uniqueErrors = {};
    ...
    entity.form.tabs.forEach((tab) => {
      tab.fields.forEach((field) => {
        ...
        const currentValue = normalizeComparable(values[field.key]);
        if (!currentValue) return;

        const duplicate = rows.some((row) => {
          const sameRecord = recordId && row.code === recordId;
          return !sameRecord && normalizeComparable(row[field.key]) === currentValue;
        });

        if (duplicate) {
          uniqueErrors[field.key] = `${field.label} must be unique.`;
          if (!firstInvalidTab) firstInvalidTab = tab.key;
        }
      });
    });

    return { uniqueErrors, firstInvalidTab };
  }
  ...
  function validate() {
    ...
    const { uniqueErrors, firstInvalidTab: firstUniqueInvalidTab } = collectUniqueFieldErrors();
    Object.assign(nextErrors, uniqueErrors);
    if (!firstInvalidTab) firstInvalidTab = firstUniqueInvalidTab;

    setErrors(nextErrors);
    if (firstInvalidTab) {
      setActiveTab(firstInvalidTab);
      setErrorBanner(`Please fix ${Object.keys(nextErrors).length} highlighted field(s).`);
      return false;
    }
    setErrorBanner("");
    return true;
  ...
  async function persist(status) {
    const submitValues = normalizeEntityValues(entity, values, { trim: true });
    const derived = entity.list.deriveRow ? entity.list.deriveRow(submitValues) : {};
    const record = { ...submitValues, ...derived, status: status || submitValues.status || "Active" };
  ...
  async function handleSaveDraft() {
    ...
    const { uniqueErrors, firstInvalidTab } = collectUniqueFieldErrors();
    if (firstInvalidTab) {
      setErrors(uniqueErrors);
      setActiveTab(firstInvalidTab);
      setErrorBanner(Object.values(uniqueErrors)[0]);
      return;
    }
