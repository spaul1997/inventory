...
const rowActionIcons = { Eye, Pencil, Check, Printer, Ban, PackageCheck };
...
export function StockList({ entityKey }) {
  ...
  function runAction(action, row) {
    setOpenMenuFor(null);
    if (action.print) {
      navigate(`/stock-management/${entityKey}/${row.id}/view`);
      return;
    }
    if (action.key === "view") {
      navigate(`/stock-management/${entityKey}/${row.id}/view`);
    ...
    if (action.key === "edit") {
      navigate(`/stock-management/${entityKey}/${row.id}/edit`);
      return;
    }
    if (action.setStatus) {
      if (action.confirm) {
    ...
    <div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <Link
            to={`/stock-management/${entityKey}/new`}
                        ...
                        return (
                          <td key={col.key} className={`px-3 py-3 first:pl-4 ${col.align === "right" ? "text-right" : ""}`}>
                            {col.badge ? (
                              ...
                              <Link
                                to={`/stock-management/${entityKey}/${row.id}/view`}
