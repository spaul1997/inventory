...
export function SalesList({ entityKey }) {
  const entity = salesEntities[entityKey];
  const { getRows, updateRow, removeRow } = useSalesData();
  ...
  function runAction(action, row) {
    setOpenMenuFor(null);
    if (action.print) {
      navigate(`/sales/${entityKey}/${keyOf(row)}/view`);
      return;
    }
    if (action.key === "view") {
      navigate(`/sales/${entityKey}/${keyOf(row)}/view`);
    ...
    if (action.key === "edit") {
      navigate(`/sales/${entityKey}/${keyOf(row)}/edit`);
      return;
    }
    if (action.key === "duplicate") {
      navigate(`/sales/${entityKey}/new`, { state: { duplicateFrom: row } });
      return;
    }
    if (action.convertsTo) {
      navigate(`/sales/${action.convertsTo}/new`, { state: { convertFrom: { entityKey, record: row } } });
      return;
    }
    if (action.remove || action.setStatus) {
      if (action.confirm) {
                        ...
                        return (
                          <td key={col.key} className={`px-3 py-3 first:pl-4 ${col.align === "right" ? "text-right" : ""}`}>
                            {col.badge ? (
                              ...
                              <Link
                                to={`/sales/${entityKey}/${rowKey}/view`}
